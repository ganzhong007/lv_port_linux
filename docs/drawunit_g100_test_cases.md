# DrawUnitG100 实施前验证用例清单

> 在编写 `draw/g100/` 代码前，用本清单定义 **验什么、怎么验、通过标准**。  
> 关联：[drawunit_g100_design.md](./drawunit_g100_design.md) §4.8（GPU 必达）、§8（实施阶段）
...
```bash
# 板级 G100 配置（示例）
cmake -B build-g100 -DCONFIG=<soc>-g100 -DLVGL_APP_DEMO=<demo>
...
| ID | P | 用例名 | 入口 | 步骤 | 通过标准 |
|----|---|--------|------|------|----------|
| G0-01 | P0 | 编译链接 | `cmake --build build-g100` | 开启 `LV_USE_DRAW_G100=1`，关 NANOVG/OPENGLES | 零 error；`lvglsim` 可执行 |
| G0-02 | P0 | Unit 注册 | 启动任意 demo | 断点或日志：`lv_draw_g100_init()` 在 `lv_opengles_init` 后调用 | G100 unit 存在；NanoVG unit 未注册 |
| G0-03 | P0 | GL context | G0-02 | `eglInitialize` / `eglMakeCurrent` 成功 | `g100_gl_ready()==true` |
| G0-04 | P1 | 能力查询 | 启动日志 | 打印 `g100_caps`：GLES 版本、FBO、max texture | 字段非零且与 `eglinfo` 一致 |
| G0-05 | P1 | FBO 池初始化 | G0-04 | 检查 `g100_fbo_pool` 预分配 | `g100_caps.fbo_ok==true`；失败时有降级重试日志 |
| G0-06 | P2 | 互斥宏 | 故意同时开 G100+NANOVG 编译 | 应编译失败或 Kconfig 互斥 | 构建系统阻止双 GPU unit |
| G0-07 | P0 | 代码拆分 | 检查 `draw/g100/` 与 `draw/nanovg/` | g100 文件 `#if LV_USE_DRAW_G100`；符号 `lv_draw_g100_*` / `lv_g100_*`；nanovg 无 G100 守卫 | 两目录独立编译；无交叉引用 |
| G0-08 | P0 | stress 不崩溃 | `./build-g100-stress/bin/lvglsim -b wayland -W 800 -H 480` + `lv_demo_stress` | 运行 ≥45s | 无 SIGSEGV；sysmon 有 FPS 样本；日志 `DrawUnitG100 ready` |
| G0-09 | P1 | stress FPS 对照 | 对比 `wayland-egl` vs `wayland-g100` | 同分辨率/时长 | g100 fps_avg 与 NanoVG 基线相当（WSLg ~175） |
...
| ID | P | 用例名 | 入口 | 步骤 | 通过标准 |
|----|---|--------|------|------|----------|
| TX-01 | P0 | 静态标签 | `simple_button` / `TEXT` scene | `text_static=1` 固定串 | 文字清晰；G100 dispatch LABEL |
...
| ID | P | 用例名 | 入口 | 步骤 | 通过标准 |
|----|---|--------|------|------|----------|
| VC-01 | P0 | VECTOR task 接管 | `lv_demo_vector_graphic_buffered` | 启动 demo | 无 `unsupported style`；preferred unit = G100 |
| VC-02 | P0 | SOLID 填充 | `test_draw_vector`（lvgl tests） | 基础路径 | GPU；截图与参考一致 |
| VC-03 | P0 | GRADIENT 填充 | vector demo 渐变路径 | 矢量渐变 | g100_grad 或 vector grad shader |
...
| ID | P | 用例名 | 入口 | 步骤 | 通过标准 |
|----|---|--------|------|------|----------|
| BL-01 | P0 | 小半径 blur | `test_draw_blur` / widgets 毛玻璃 | radius 8～32 | 模糊可见；G100 BLUR task |
...
| D3-07 | P1 | `render_cb` 自定义 mesh | `lv_3dview_set_render_cb` | 回调内 `glDraw*` | 每帧 FBO 更新；G100 `lv_draw_g100_3d` composite |
...
> **3D Draw Task 全族规格：** [g100_3d_draw_tasks_design.md](./g100_3d_draw_tasks_design.md)。D3-06～10 为 widget 功能；D3-16～19 为 task 管线验收。

---

## 9. SW 兜底验证（须确认仍走 SW）

> 这些用例 **预期 G100 evaluate=0**，由 `draw/sw` 完成，结果正确即可。

| ID | P | 用例名 | 入口 | 步骤 | 通过标准 |
|----|---|--------|------|------|----------|
| SW-01 | P0 | MASK_BITMAP | `test_draw_layer` bitmap_mask | `bitmap_mask_src` 样式 | 蒙版形状正确；G100 未接 task |
| SW-02 | P0 | Canvas CPU | `lv_canvas` + `lv_canvas_init_layer` | 画 fill/label | 全程 SW；apitrace 无 G100 主屏 bind |
...
| ID | P | 用例名 | 入口 | 步骤 | 通过标准 |
|----|---|--------|------|------|----------|
| PF-01 | P0 | stress 短跑 | `scripts/benchmark_stress_shm_vs_egl.sh` 60s | G100 build | 无 crash；sysmon FPS 稳定 |
| PF-02 | P0 | benchmark | `LVGL_APP_DEMO=benchmark` | 全场景 | CSV render/flush 有数据；G100 ≥ SW 或 CPU↓ |
...
| AP-04 | P1 | 渐变 shader | `lv_demo_render` LINEAR | `glUseProgram` 命中 g100_grad |
...
| 阶段 | 最少通过用例 | 门禁 |
|------|-------------|------|
| **G0** | G0-01～03, G0-07～08 | 可启动；`draw/g100/` 独立；stress 不崩溃 |
...
| `test_render_to_*.c` | SW-04（CPU canvas，非 G100 主路径） |
| `test_demo_render.c`（若有） | GR-* / D2-* 全场景 |

**说明**：lvgl 单测默认 **SW/离屏 canvas**，多数字面验证 **逻辑正确性**；G100 专项须在 **`lvglsim` + EGL** 下跑 §3～11 中带 apitrace/sysmon 的用例。
